  ...
  for (m in 1: length(method_ord)){
    for (cross in 1:5){
      eff_list <- alply(c(1: 22), 1, function (a) {
                        read.table(paste0("/net/mulan/disk2/yasheng/out_bbj/pheno", pheno[p], 
...
save(eff_summ, file = "/net/mulan/disk2/yasheng/out_bbj/summ.RData")
...
### total
load("/net/mulan/disk2/yasheng/out_bbj/summ.RData")
r2_list_bbj <- eff_summ
r2_mean_bbj <- laply(r2_list_bbj, function(a) colMeans(a, na.rm = T))
load("/net/mulan/disk2/yasheng/out_sample/summ.RData")
r2_list_eur <- eff_summ
r2_mean_eur <- laply(r2_list_eur, function(a) colMeans(a, na.rm = T))
load("/net/mulan/home/yasheng/Biobank/code/summ_res/r2_list_c.RData")
r2_mean <- laply(r2_list, function(a) colMeans(a, na.rm = T))
r2_mean <- r2_mean[c(1, 4, 10, 11, 12, 13), ]

max(1 - r2_mean_bbj[, 1] / r2_mean[, 1])
min(1 - r2_mean_bbj[, 1] / r2_mean[, 1])
mean(1 - r2_mean_bbj[, 1] / r2_mean[, 1])
max(1 - r2_mean_eur[, 1] / r2_mean[, 1])
min(1 - r2_mean_eur[, 1] / r2_mean[, 1])
mean(1 - r2_mean_eur[, 1] / r2_mean[, 1])