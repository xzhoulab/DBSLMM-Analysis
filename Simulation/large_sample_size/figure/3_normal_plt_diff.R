...
r2_SLMM_diff_plt23 <- ggplot(r2_SLMM_diff_dat23, aes(x=Method, y=R2, fill=Method))+
  ...
  theme(axis.text.x = element_blank(), 
        ...
        plot.tag = element_text(size = 50, face = "bold"),
        strip.text = element_text(face = 'bold', size = rel(3)))+ 
  scale_y_continuous(breaks = seq(-0.3, 0.3, 0.15), limits = c(-0.35, 0.3))+
  geom_hline(aes(yintercept = 0), color="#EFC000FF",
             linetype="dashed", size=1.5, na.rm = T) +
  ylab(expression(italic(R^2) ~ "Difference"))+
  labs(tag = "D", title = "Hybrid II")
...
## Output
library(patchwork)
tiff("/net/mulan/disk2/yasheng/figures/simulation/diff_normal_herit0.1.tiff", height = 1200, width = 1600)
(r2_SLM_diff_plt1 | r2_LMM_diff_plt1) /
  (r2_SLMM_diff_plt11 | r2_SLMM_diff_plt21)
dev.off()
tiff("/net/mulan/disk2/yasheng/figures/simulation/diff_normal_herit0.2.tiff", height = 1200, width = 1600)
(r2_SLM_diff_plt2 | r2_LMM_diff_plt2) /
  (r2_SLMM_diff_plt12 | r2_SLMM_diff_plt22)
dev.off()
tiff("/net/mulan/disk2/yasheng/figures/simulation/diff_normal_herit0.5.tiff", height = 1200, width = 1600)
...
## summary of DBSLMM
summ <- function(dat){
  mean_x <- round(mean(as.numeric(dat$R2[dat$Method == "DBSLMM"])), 3)
  sd_x <- round(sd(as.numeric(dat$R2[dat$Method == "DBSLMM"])), 3)
  cat (mean_x, sd_x, "\n")
}
summ(r2_SLM_dat1)
summ(r2_LMM_dat1)
